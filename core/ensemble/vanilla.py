...
class Vanilla(Ensemble):
    ...
    def evaluate(self, x, gt_labels, threshold=0.5, name='test'):
        ...
        if not is_single_class:
            ...
            return MSG.format(fnr * 100, fpr * 100, f1 * 100) + "The balanced accuracy on the {} dataset is {:.5f}%".format(name, accuracy * 100)
    ...
    def fit(self, train_set, validation_set=None, input_dim=None, EPOCH=20,**kwargs):
        """
        ...
        train_loss_list = []
        val_loss_list = []
        for epoch in range(EPOCH): #self.hparam.n_epochs
            train_acc = 0.
            val_acc = 0.
            train_loss = 0.
            val_loss = 0.
    ...
    def load_ensemble_weights(self):
        ...
        print(path.exists(path.join(self.save_dir, self.architecture_type)))
        print(path.join(self.save_dir, self.architecture_type))
