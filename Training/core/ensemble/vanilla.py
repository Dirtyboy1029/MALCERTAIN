...
class Vanilla(Ensemble):
    ...
    def save_ensemble_weights(self, save_path=None):
        ...
        if not save_path:
            ...
            utils.dump_joblib(self._optimizers_dict,
            ...
            if not path.exists(path.join(save_path, self.architecture_type)):
                utils.mkdir(path.join(save_path, self.architecture_type))
            self.base_model.save(path.join(save_path, self.architecture_type))
            print("Save the model configuration to directory {}".format(save_path))

            # save model weights
            utils.dump_joblib(self.weights_list, path.join(save_path, self.architecture_type + '.model'))
            utils.dump_joblib(self._optimizers_dict,
                              path.join(save_path, self.architecture_type + '.model.metadata'))
            print("Save the model weights to directory {}".format(save_path))
            return
